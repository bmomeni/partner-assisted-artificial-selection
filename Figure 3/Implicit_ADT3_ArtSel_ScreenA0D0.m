% Partners Assisted Artificial Selection (c) 2022 The Trustees of Boston College. 
% Licensed under the Apache License, Version 2.0 (the "License") with Commons Clause Restriction v1.0; you may not use this file except in compliance with the License. 
...
iTm = 0.01; % inhibitory coefficient of T on A, ml/(ug.hr) %%set value
...
for A0 = A0rng
    ...
    for D0 = D0rng
        ...
        for ii = 1:Ni
            ...
            KA0 = KA0m; %random('Normal',KA0m,0.1*svar*KA0m,[1,Ne]); % A carrying capacity %set value
            rD0 = skewnormal(rD0m,0.1*svar*rD0m,-3,[1,Ne]); % max D growth rate, 1/hr %%evolving
            KD0 = KD0m; %random('Normal',KD0m,0.1*svar*KD0m,[1,Ne]); % D carrying capacity %%evolving
